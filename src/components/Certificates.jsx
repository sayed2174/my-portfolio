      ...
      <div className="w-full p-4 md:p-8 lg:p-12">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
          {certificates.map((cert, idx) => (
            <motion.div
              key={idx}
              // Use the link from the certificate object
              onClick={() => window.open(cert.link, "_blank")}
              className="p-3 rounded-xl shadow-md cursor-pointer transform hover:scale-105 transition-all duration-300 text-center"
              style={{
              ...
              viewport={{ once: false }}
            >
              {/* Use the name from the certificate object */}
